...
function OutputOptions({ processing }) {
  const { t } = useI18n();
  const { burn, format, matchingVoices, route, running, setBurn, setSpeed, setSubtitleContent, setVoice, speed, subtitleContent, ttsMode, voice, voiceError } = processing;
  ...
  return <>
    ...
    {ttsMode === 'qwen' ? <p className="app-note">{t('qwenFixedSpeed')}</p> : <label><span className="mb-1 flex justify-between text-sm"><span>{t('speechSpeed')}</span><strong>{speed.toFixed(2)}x</strong></span><input className="w-full accent-indigo-600" type="range" min="0.8" max="1.2" step="0.05" value={speed} onChange={e => setSpeed(Number(e.target.value))} disabled={running} /></label>}
    {voiceError && <p role="alert" className="rounded-xl border border-red-300 bg-red-50 px-3 py-2 text-sm text-red-800 dark:border-red-700 dark:bg-red-950/30 dark:text-red-200">{voiceError}</p>}
  </>;
}
...
export default function ProcessPanel(props) {
  ...
  return <section className="app-panel overflow-hidden">
    ...
    <div className="app-process-body space-y-4 p-4">
      ...
      {route === 'dubbing' && onlineLanguages.includes(languageCode(targetLang)) && <p role="note" className="rounded-xl border border-amber-300 bg-amber-50 px-3 py-2.5 text-sm text-amber-900 dark:border-amber-700 dark:bg-amber-950/40 dark:text-amber-100">{t('edgeTtsPrivacyNotice')}</p>}
      <OutputOptions processing={processing} />
      <ProcessingStatus processing={processing} />
      <button type="button" className="app-primary-button app-process-primary" onClick={() => run()} disabled={running || !task.transcription_path || (route === 'dubbing' && !voice)}>{running ? t('processing') : route === 'dubbing' ? t('createDubbedVideo') : burn ? t('createHardSubtitleVideo') : t('createSubtitleFile')}</button>
